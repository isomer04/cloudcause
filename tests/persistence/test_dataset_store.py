...
def test_the_http_topology_with_a_database_gets_the_sql_store(tmp_path: Path) -> None:
    settings = sql_settings(tmp_path)
...
def test_a_dataset_survives_a_gateway_restart_with_sql_storage(tmp_path: Path) -> None:
    settings = sql_settings(tmp_path)
    store = build_dataset_store(settings)
...
def test_sql_and_memory_agree_on_what_a_dataset_looks_like(tmp_path: Path) -> None:
    memory_settings = Settings.from_env({})
    memory = MemoryDatasetStore(memory_settings)
    dataset_id = fill(memory, memory_settings)

    sql = build_dataset_store(sql_settings(tmp_path))
