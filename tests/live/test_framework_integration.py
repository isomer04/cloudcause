...
async def test_all_three_frameworks_participate_in_one_investigation(
    ...
    assert live_providers, "no framework ran in live mode; check the extras and API keys"
...
async def test_live_summary_is_written_without_new_numbers(live_settings: Settings) -> None:
    request = InvestigationRequest(
        providers=["aws"],
        start_date=date(2026, 7, 13),
        end_date=date(2026, 7, 19),
    ...
    assert "126" in report.summary or "126.00" in report.summary
