...
def test_a_daily_grain_aws_export_does_not_lose_its_last_day(upload_settings: Settings) -> None:
    """The partial-day bar is the file's own grain, not a hard-coded 24.

    A daily-grain export carries one bucket on every date including the last.
    Measured against 24 that final day was always "1 of 24 hourly buckets", so every
    such upload surrendered its most recent day to a caveat that described the file
    format rather than any gap in the data.
    """

    parsed = parse_cost_source(
        "aws", aws_cur_json(daily_grain=True, last_day_hour=23), upload_settings
    )

    assert parsed.data_through is not None
    assert parsed.data_through.date() == CURRENT_END
    assert "1 of 24" not in parsed.data_through_note
    assert "rather than hourly detail" in parsed.data_through_note
    assert "provisional" in parsed.data_through_note


def test_a_daily_grain_export_measures_the_same_total_as_the_hourly_one(
    upload_settings: Settings,
) -> None:
    """Grain changes the caveat, never the money."""

    hourly = parse_cost_source("aws", aws_cur_json(), upload_settings)
    daily = parse_cost_source("aws", aws_cur_json(daily_grain=True), upload_settings)

    assert sum(record.effective_cost for record in daily.costs) == pytest.approx(
        sum(record.effective_cost for record in hourly.costs), abs=0.01
    )
    assert daily.period_start == hourly.period_start
    assert daily.period_end == hourly.period_end


def test_an_hourly_export_still_detects_a_genuinely_cut_final_day(
    upload_settings: Settings,
) -> None:
    """The grain-relative bar must not weaken the case the check exists for."""

    parsed = parse_cost_source("aws", aws_cur_json(hours_on_last_day=5), upload_settings)

    assert parsed.data_through is not None
    assert parsed.data_through.date() == date(2026, 7, 18)
    assert "5 of 24" in parsed.data_through_note


def test_one_short_earlier_day_does_not_redefine_the_grain(upload_settings: Settings) -> None:
    """The bar is the fullest earlier day, not a typical one.

    An hourly export with one truncated day earlier in the file still describes
    hourly data. Averaging the earlier days would let that one short day drag the
    bar below 24, reclassify the file as coarse-grained, and wave the genuinely
    cut final day through as complete.
    """

    rows = aws_cur_rows(hours_on_last_day=5)
    short_day = date(2026, 7, 9).isoformat()
    trimmed = [
        row
        for row in rows
        if not (
            str(row["line_item_usage_start_date"]).startswith(short_day)
            and int(str(row["line_item_usage_start_date"])[11:13]) >= 3
        )
    ]
    assert len(trimmed) < len(rows), "the fixture must actually lose hours on that day"

    parsed = parse_cost_source("aws", json.dumps({"rows": trimmed}).encode(), upload_settings)

    assert parsed.data_through is not None
    assert parsed.data_through.date() == date(2026, 7, 18), (
        "the final day is still short against the fullest earlier day, so it is still partial"
    )
    assert "5 of 24" in parsed.data_through_note
