...
import cloudcause_orchestrator.live_agent as live_agent
import cloudcause_orchestrator.orchestrator as orchestrator_module
import pytest
...
async def test_a_residual_survives_reconciliation_on_every_provider(
    report: InvestigationReport,
) -> None:
    """The tolerance band has to be observable, not merely declared.

    The fixture plants diffuse sub-materiality drift on one untagged SKU per
    provider, so reconciliation lands inside tolerance with something left over
    instead of on exactly zero. A run that attributes 100.00% of every provider's
    change to the cent is the shape of an answer written backwards from the
    question, and it never exercises the band at all.
    """

    assert report.reconciliation is not None
    assert report.reconciliation.unattributed_change > 0.0
    assert report.reconciliation.within_tolerance is True
    for provider in report.comparison.providers:
        assert provider.reconciliation.unattributed_change > 0.0, provider.provider
        assert provider.reconciliation.within_tolerance is True, provider.provider


async def test_planted_drift_never_becomes_a_finding(report: InvestigationReport) -> None:
    """Drift under the materiality floor must stay unattributed, not get a cause."""

    drifting = {"AmazonCloudWatch", "Bandwidth", "Networking"}
    for finding in report.findings:
        assert finding.service_name not in drifting, finding.finding_id
...
async def test_report_renders_to_markdown_with_provenance(report: InvestigationReport) -> None:
    ...
    assert "**investigation mode:** deterministic playbooks" in markdown
...
async def test_report_publishes_only_the_deterministic_executive_summary(
    multi_cloud_request: InvestigationRequest,
    settings: Settings,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    marker = "deterministic evidence-backed executive summary"
    monkeypatch.setattr(orchestrator_module, "_deterministic_summary", lambda *_args: marker)

    report = await Orchestrator(settings).run(multi_cloud_request)

    assert report.summary == marker
    assert not hasattr(live_agent, "synthesize_summary")
