...
def aws_cur_rows(
    *,
    ...
    """Hourly CUR 2.0 rows: a flat EC2 baseline plus a NAT Gateway spike."""
    ...
    for day, in_current in all_days:
        hours = hours_on_last_day if day == last_day else 24
        for hour in range(hours):
            for entry in series:
                cost = entry["current_hourly_cost"] if in_current else entry["baseline_hourly_cost"]
                quantity = (
