...
async def test_request_selects_live_mode_without_restarting_worker(
    settings: Settings, monkeypatch: pytest.MonkeyPatch
) -> None:
    investigator = AwsInvestigator(settings)
    request = await worker_request("aws", settings)
    request.request.agent_mode = "live"
    saw_live_context = False

    async def fake_live(ctx):
        nonlocal saw_live_context
        saw_live_context = ctx.settings.agent_mode == "live"
        return await investigator.run_stub(ctx)

    monkeypatch.setattr(investigator, "run_live", fake_live)
    response = await investigator.investigate(request)

    assert saw_live_context is True
    assert response.agent_mode == "live"
    assert response.status == "ok"


async def test_live_call_budget_exhaustion_falls_back_to_playbooks(
    settings: Settings, monkeypatch: pytest.MonkeyPatch
) -> None:
    investigator = AwsInvestigator(settings.with_overrides(max_agent_calls=1))
    request = await worker_request("aws", investigator.settings)
    request.request.agent_mode = "live"

    async def exhaust_budget(ctx):
        ctx.reserve_agent_call("model:openai:strands")
        ctx.reserve_agent_call("native_tool:get_candidate_evidence")
        return []

    monkeypatch.setattr(investigator, "run_live", exhaust_budget)
    response = await investigator.investigate(request)

    assert response.status == "partial"
    assert response.agent_mode == "stub"
    assert response.findings
    assert any("call budget exhausted" in warning for warning in response.warnings)


async def test_context_failure_reports_that_no_live_agent_executed(
    settings: Settings, monkeypatch: pytest.MonkeyPatch
) -> None:
    investigator = AwsInvestigator(settings)
    request = await worker_request("aws", settings)
    ...
    async def fail_context(_request):
        raise RuntimeError("dataset unavailable")

    monkeypatch.setattr(investigator, "build_context", fail_context)
    response = await investigator.investigate(request)
