...
def test_the_child_resolves_the_dataset_its_environment_names(monkeypatch: pytest.MonkeyPatch, upload_settings) -> None:
...
def _sealed_upload(upload_settings):
    from cloudcause_datasets import add_source, build_dataset_store, parse_cost_source, seal_dataset
    from conftest import aws_cur_json
...
def test_stdio_child_resolves_an_inprocess_dataset_from_the_snapshot(
    monkeypatch: pytest.MonkeyPatch, upload_settings
) -> None:
    from pathlib import Path

    from cloudcause_mcp import cleanup_server_snapshot
    from cloudcause_providers import UploadDataProvider

    dataset = _sealed_upload(upload_settings)
    params = operational_server_params(
        "aws", "", dataset.dataset_id, snapshot_dataset=True, settings=upload_settings
    )
    snapshot = Path(params["env"]["CLOUDCAUSE_DATASET_SNAPSHOT"])
    ...
    _, tools = build_operational_server()
    assert isinstance(tools._adapter, UploadDataProvider)
    assert tools._adapter.dataset.dataset_id == dataset.dataset_id

    # The parent owns the snapshot's lifetime; a child that consumed it would
    # break the next child a reconnecting framework starts.
    assert snapshot.is_file(), "the child must not consume a handoff it may need again"
    _, again = build_operational_server()
    assert again._adapter.dataset.dataset_id == dataset.dataset_id

    cleanup_server_snapshot(params)
    assert not snapshot.exists(), "the parent must not leave billing data on disk"


def test_stdio_child_falls_back_when_the_snapshot_is_already_gone(
    monkeypatch: pytest.MonkeyPatch, upload_settings
) -> None:
    """A vanished snapshot must degrade to the dataset id, not abort startup."""
    ...
    for key, value in params["env"].items():
        monkeypatch.setenv(key, value)
    cleanup_server_snapshot(params)
    assert not Path(params["env"]["CLOUDCAUSE_DATASET_SNAPSHOT"]).exists()
