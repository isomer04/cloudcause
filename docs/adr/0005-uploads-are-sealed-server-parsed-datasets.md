...
* Status: Accepted
* Date: 2026-07-28
* Scope: `packages/datasets`, `packages/focus`, `api`, `web`
...
orchestrator, in the worker engine, and in `packages/mcp/tools.py` — where
...
`packages/focus` parsers a live provider connector will use —
...
aggregated to the daily grain `packages/anomaly` consumes, and discarded.
