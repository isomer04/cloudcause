...
class OperationalDataTools:
    ...
    def __init__(
        ...
        self._adapter = get_data_provider(provider, self.settings, scenario_id, dataset_id)
    ...
    async def get_cost_breakdown(
        ...
        baseline = DateRange(
            start=date.fromisoformat(baseline_start), end=date.fromisoformat(baseline_end)
        )
        result = await self._adapter.get_costs([current, baseline])
        dimension: Dimension = group_by if group_by in {
            "service",
            "region",
            "account",
            "resource",
            "tag_owner",
        } else "service"  # type: ignore[assignment]
    ...
    async def get_audit_events(
        self, start: str, end: str, resource_id: str | None = None
    ) -> dict[str, Any]:
...
class BillingKnowledgeTools:
    ...
    def get_provider_data_freshness_rules(
        self, provider: str, usage_date: str | None = None
    ) -> dict[str, Any]:
