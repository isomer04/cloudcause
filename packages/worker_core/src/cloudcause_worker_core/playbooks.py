...
def _score_confidence(
    ...
    confidence = 0.30
    confidence += 0.10 if "usage" in kinds else 0.0
    confidence += 0.15 if "metric" in kinds else 0.0
    confidence += 0.20 if "audit" in kinds else 0.0
    confidence += 0.10 if "inventory" in kinds else 0.0
    confidence += 0.10 if "recommendation" in kinds else 0.0
    if candidate.is_new:
        confidence += 0.05
