...
def _observed_grain(counts: Sequence[int]) -> int:
    """What a complete day looks like in this file: the fullest day seen.

    A day's bucket count is distinct hour-of-day values, so it can never exceed
    24 and a complete day reaches exactly that. The maximum is therefore the
    right estimator and is robust in the only direction that matters: a quiet or
    truncated day lowers a count and so cannot drag the bar down, whereas a
    median would. Two earlier days of 5 and 24 buckets describe an hourly export
    with one truncated day, not a 5-row-per-day export.
    """
...
def _hour_buckets(rows: Sequence[Mapping[str, Any]]) -> dict[date, set[int]]:
    buckets: dict[date, set[int]] = {}
    for row in rows:
        stamp = _as_datetime(row.get("line_item_usage_start_date"))
        if stamp is not None:
            buckets.setdefault(stamp.date(), set()).add(stamp.hour)
    return buckets
...
def _aws_data_through(
    ...
    """A short final day is visibly incomplete - but only against the file's own grain.

    A real Cost and Usage Report is hourly, so a last date carrying 5 of 24 start
    hours was cut mid-day. That question can only be asked before aggregation: once
    rows collapse to a daily grain, a partial day is indistinguishable from a quiet one.

    Measuring against a constant 24 was wrong for the other common shape. A
    daily-grain export - one row per resource per day, which is what Cost Explorer
    and most hand-built exports produce - carries exactly one bucket on every date
    including the last. Against a hard 24 that file's final day is always "1 of 24
    hourly buckets", so every daily-grain upload lost its last day to a caveat that
    described the file's format rather than any gap in it. The bar is therefore the
    fullest other day in this same file: a complete day is whatever a complete day
    looks like here.
    """
    ...
    observed = len(buckets.get(last_date, ()))
    if observed >= HOURS_PER_DAY:
        return _end_of_day(last_date), ""

    earlier = sorted(day for day in buckets if day < last_date)
    if not earlier:
        return _end_of_day(last_date), (
            f"{last_date.isoformat()} carries {observed} of {HOURS_PER_DAY} hourly buckets and is "
            "the only date in the file, so there is no other day to compare it against. Coverage "
            "is reported through it but treated as provisional."
        )

    expected = _observed_grain([len(buckets[day]) for day in earlier])
    if expected < HOURS_PER_DAY:
        return _end_of_day(last_date), (
            f"This export carries {expected} row(s) per day rather than hourly detail, so a "
            f"partial final day is indistinguishable from a quiet one. {last_date.isoformat()} is "
            "taken as complete. If that day was still accruing, treat conclusions about it as "
            "provisional."
        )
    if observed >= expected:
        return _end_of_day(last_date), ""
    return _end_of_day(max(earlier)), (
        f"{last_date.isoformat()} carries only {observed} of {expected} hourly buckets seen on "
        f"every other day in this file, so it is a partial day. Coverage is reported through "
        f"{max(earlier).isoformat()}; the missing hours are unavailable data, never zero usage."
    )
