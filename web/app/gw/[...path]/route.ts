...
import { GATEWAY_URL } from "@/lib/gateway-server";
...
const GET_PATTERNS: RegExp[] = [
  ...
  new RegExp(
    String.raw`^api/v1/investigations/${INVESTIGATION}/(events|progress|report|report\.md|report\.pdf|wait)$`,
  ),
...
const FORWARDED_RESPONSE_HEADERS = [
  ...
  "cache-control",
  "content-disposition",
  // The 429 rate-limit contract: the browser client reads retry_after_seconds
  // from the JSON body, but the header is what a non-browser HTTP client (or
  // a proxy/CDN in front of this one) would honor.
  "retry-after",
];
