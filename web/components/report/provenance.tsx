...
import { day, delta, duration, money, stamp } from "@/lib/format";
    ...
    <section aria-label="Provenance and freshness">
      ...
      <div className={`grid gap-4 ${compact ? "grid-cols-1" : "lg:grid-cols-2"}`}>
          ...
          {compact ? (
            <ul className="divide-y divide-rule">
              {report.provider_statuses.map((status) => (
                <li key={status.provider} className="px-3 py-3 text-xs">
                  <div className="flex items-center justify-between gap-2">
                    <ProviderMark provider={status.provider} />
                    <WorkerStatusText status={status.status} />
                  </div>
                  {status.message ? <p className="mt-2 leading-relaxed text-ink-soft">{status.message}</p> : null}
                  <p className="num mt-2 text-[0.6875rem] text-ink-mute">{status.finding_count} findings · {duration(status.duration_seconds)} · through {stamp(status.data_through)}</p>
                </li>
              ))}
            </ul>
        ...
        <div className="panel overflow-hidden">
          ...
          <dl className={`divide-y divide-rule text-sm ${compact ? "[&_dd]:w-full [&_dd]:text-left" : ""}`}>
