...
from cloudcause_rate_limit import AIRequestGovernor, build_rate_limiter
...
def build_worker_clients(
    ...
    # One governor shared by every in-process investigator this call builds,
    # so AWS and Azure draw from the same OpenAI concurrency/rate bucket
    # instead of each getting its own private allowance. HTTP-topology workers
    # are separate processes and build their own governor at their own entry
    # point; only a Redis backend lets those share state with each other.
    governor = AIRequestGovernor(build_rate_limiter(settings), settings)
    clients: dict[Provider, WorkerClient] = {
        "gcp": InProcessWorkerClient(GcpInvestigator(settings, knowledge, governor))
    }
    ...
    clients["aws"] = InProcessWorkerClient(AwsInvestigator(settings, knowledge, governor))
    clients["azure"] = InProcessWorkerClient(AzureInvestigator(settings, knowledge, governor))
    return clients
