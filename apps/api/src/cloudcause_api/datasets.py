...
file that matters would land on disk, and ``apps/api`` does not declare
...
def describe() -> dict[str, object]:
    ...
    if state.store.kind == "memory" and state.settings is not None:
        return build_dataset_store(state.settings).describe()
    return state.store.describe()
...
async def seal(dataset_id: str) -> DatasetSummary:
    ...
    store = _store()
    try:
        dataset = seal_dataset(store, dataset_id)
    except (DatasetError, IngestError) as error:
        raise _fail(error) from error
...
def require_runnable_dataset(dataset_id: str) -> None:
