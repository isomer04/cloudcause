...
import { safeHealth } from "@/lib/gateway";
...
export async function Rail() {
  ...
  return (
    <div className="bg-brand-deep text-on-brand lg:sticky lg:top-0 lg:z-[var(--z-rail)] lg:flex lg:h-dvh lg:flex-col">
      <div className="flex items-center justify-between gap-4 px-5 py-4 lg:block lg:px-6 lg:py-7">
        <Link href="/" className="group block">
          <span className="block font-display text-[1.4rem] leading-none tracking-[-0.01em] text-on-brand">
            CloudCause
          </span>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-white/12 px-5 pb-3 pt-2.5 text-[0.6875rem] text-on-brand-mute lg:hidden">
        ...
        {health ? (
          <>
            <span aria-hidden>·</span>
            <span className="num">{health.agent_mode} agents</span>
          </>
        ) : null}
      </div>

      <div className="hidden border-t border-white/12 px-6 py-5 lg:mt-auto lg:block">
        <dl className="space-y-2.5 text-[0.6875rem]">
          ...
          {health ? (
            <>
              ...
              <div className="flex items-baseline justify-between gap-3">
                <dt className="text-on-brand-mute">Agents</dt>
                <dd className="num text-on-brand">{health.agent_mode}</dd>
              </div>
              <div className="flex items-baseline justify-between gap-3">
            ...
            <p className="text-on-brand-mute">
              Start it with <span className="num">uv run cloudcause-api</span>.
            </p>
          )}
        </dl>
